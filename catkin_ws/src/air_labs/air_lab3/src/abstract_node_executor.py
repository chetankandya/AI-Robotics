...
class EchoExecutor(TstML.Executor.AbstractNodeExecutor):
    ...
    def start(self):
        ...
        msg.data = self.node().getParameter(TstML.TSTNode.ParameterType.Specific, "text")

        finite_loop = self.node().hasParameter(TstML.TSTNode.ParameterType.Specific, "count")
        loop_count = self.node().getParameter(TstML.TSTNode.ParameterType.Specific, "count")
